...
class Generator(agg: Aggregator, params: Params, lstm: CharPredictor) {
  ...
  def getRandomStart: String = {
    val rand = new Random()
    val validTweets = agg.filter(_.length > params.getBatchSize)
    //val tweet = validTweets(rand.nextInt(validTweets.length))
    val tweet = validTweets(0)
    tweet.substring(0, params.getBatchSize) +
    tweet.substring(params.getBatchSize, tweet.length).split(" ")(0) + " "
  }
  ...
  def next: Array[Double] = {
    val tail = output.substring(output.length - params.getBatchSize, output.length)
